...
document.addEventListener("DOMContentLoaded", function() {
    const names = ["Joao", "Pedro", "Lucas", "Jorge"];
    ...
    addBtn.addEventListener("click", () => {
        ...
        // escolhendo nome e número
        const name = names[Math.floor(Math.random() * names.length)];
        const number = Math.floor(Math.random() * 10);

        // criando novas células na tabela
        const cell1 = newRow.insertCell(0);
        const cell2 = newRow.insertCell(1);
        cell1.textContent = name; // setando nome
        cell2.textContent = number; // Setando número
    });

    delBtn.addEventListener("click", () => {
        if (table2.rows.length > 1) {
            table2.deleteRow(-1); // deletando célula final da tabela caso tenha mais de 1 célula
        }
    });
});
